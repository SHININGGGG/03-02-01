...
/*
 * Vincent's telemetry data storage type
 */
typedef enum
{
STOP=0,
...
void sendStatus()
{
  ...
  //
   TPacket statsPacket;
  statsPacket.packetType = PACKET_TYPE_RESPONSE;
  statsPacket.command = RESP_STATUS;
  //init array
  unsigned long my_stats[10] = {leftForwardTicks,rightForwardTicks,leftReverseTicks,rightReverseTicks,leftForwardTurns,rightForwardTurns,
  leftReverseTurns,rightReverseTurns,forwardDist,reverseDist};
  memcpy(statsPacket.params, my_stats , sizeof(my_stats) );
  sendResponse(&statsPacket);  
}
...
{
  dir = FORWARD; //set the direction "dir is teh object name"
...
{
  dir = BACKWARD; //set the direction "dir is teh object name"
...
{
  dir = LEFT; //set the direction "dir is teh object name"
  int val = pwmVal(speed);
...
{
  dir = RIGHT; //set the direction "dir is teh object name"
  int val = pwmVal(speed);
...
void stop()
{
  dir = STOP; //set the direction "dir is teh object name"
  analogWrite(LF, 0);
  analogWrite(LR, 0);
  analogWrite(RF, 0);
  analogWrite(RR, 0);
